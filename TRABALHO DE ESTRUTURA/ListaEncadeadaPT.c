...
int validar_data_formatada(const char* data) {
    if (strlen(data) != 22) return 0;
    return data[13] == ':' && data[16] == ':' && data[14] == '0' && data[15] == '0' && data[17] == '0' && data[18] == '0';
}

int ja_existe(Node* head, const char* data) {
    while (head != NULL) {
        if (data_igual(head->dado.data, data)) {
            return 1;
        }
        head = head->next;
    }
...
Node* carregar_dataset(const char* nome_arquivo, Node* lista) {
    ...
    while (fgets(linha, MAX_LINHA, fp)) {
        EletricDates dado;
        memset(&dado, 0, sizeof(EletricDates));

        char* token = strtok(linha, ",");
        if (!token) continue;

        strncpy(dado.data, token, sizeof(dado.data) - 1);
        dado.data[sizeof(dado.data) - 1] = '\0';

        for (int pos = 1; (token = strtok(NULL, ",")) != NULL; pos++) {
...
int main() {
    ...
    while (1) {
        printf("\nEscolha uma opcao:\n");
        printf("1 - Buscar dado por data (YYYY-MM-DD HH:00:00 AM/PM)\n");
        printf("2 - Inserir novo dado ao fim da lista\n");
        printf("3 - Remover dado por data\n");
        ...
        switch(opcao[0]) {
            ...
            case '2': {
                EletricDates dado;
                printf("Digite a data (YYYY-MM-DD HH:00:00 AM/PM): ");
